...
<s:form  method="POST" id="form" action="inviteFriends">
 <table width="680" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr><td>选择邀请场次：<s:select name="sessionId" list="competitionSessions" listKey="key"
                    listValue="value"/></td></tr>
  <s:iterator value="myFriends" status="st">
    <s:if test="#st.index%4 == 0">
  <tr>   <td>
    </s:if>
        <s:checkbox  name="checkBox[%{#st.index}]" value="false"/>
        <input type="hidden" name="invitedUsers[<s:property value="%{#st.index}"/>]" value="<s:property value="id"/>"/>
