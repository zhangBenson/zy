...
<div id="prTipWrap" class="fl">
    ...
    <table>
        <tbody>
            <s:iterator value="meMonitorAuthorizeTo">
                <tr>
                    <td><a href="userBlog.html?user.id=<s:property value="authorizeTo.id"/>" target="_blank"><img src="<s:property value="authorizeTo.pic"/>" alt="<s:property value="authorizeTo.nickName"/>"/></a></td>
                    <td><a href="userBlog.html?user.id=<s:property value="authorizeTo.id"/>" target="_blank" class="other_more"><s:property value="authorizeTo.nickName"/></a></td>
                    <td><a onclick="cancelAuthority(this,<s:property value="authorizeTo.id"/>);" class="other_more"><s:property value="%{getText('authorization.cancel')}"/></a></td>
                </tr>
            </s:iterator>
        </tbody>
    </table>
    ...
    <div class="tipHeader fl">
        <h3 class="h3_title"><s:property value="%{getText('authorization.new')}"/></h3>(<s:property value="%{getText('pls.valid.email')}"/>)&nbsp;&nbsp;
    ...
    <s:form method="post" id="authorizeForm">
        <ul class="fl" id="authorityList">
                <li><input name="authorityToEmails" type="text" class="authorityToEmails" onblur="checkInvitedEmail(this);" />&nbsp;&nbsp;&nbsp;<span class="tip_words" id="emailTip0"></span></li>
                <li><input type="button" class="submit_btn_large" id="authorityConfirm" onclick="confirmAuthorization();" value="<s:property value="%{getText('authorization.confirm')}"/>"/></li>
        </ul>
    </s:form>
