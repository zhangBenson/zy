...
<style type="text/css">
body,div,ul,li,h1,h2,h3,h4,h5,h6,table{ margin:0px; padding:0px; font-family:Verdana, Geneva, sans-serif; color:#666; font-size:12px;margin-left: auto;margin-right: auto;}
ul,li{ list-style:none;}
#courseconfirm{ width:800px;margin-top: 10px;}
#courseconfirm div{ float:left;}
#courseconfirm a{ color:#cc6600; text-decoration:underline;}
#courseconfirm a:hover{ color:#F90;}
#blogindex{ width:1000px; margin:0px auto; position:relative; float:none; font-size:14px; text-align:left;}
.blogframe{ width:800px; background:url(images/courseconfirm/courseframecenter.gif) center repeat-y; position:relative; padding-bottom:15px;}
.blogframe #video{float:left;width:140px;padding: 2px;margin: 0 10px 20px 15px;}
.blogframe #video img{float:left;height:90px;width:120px;padding:2px;border: 1px solid #CCC;cursor: pointer;}
.blogframe #video a{float:left;width:130px;margin-top: 5px;}
.courseinfo{ width:180px;text-align: left;}
.courseinfo p{ margin:0; line-height:24px;}
b{ color:#cc6600;}
h6{ color:#006699; font-size:18px;padding-bottom:2px; float:left;margin-right:70% ;margin-top: 15px;margin-bottom: 5px;}
...
.reservationconfirmed{ border:none;height:40px; width:136px; cursor:pointer; margin:5px 30px 0px 15px;background:url(images/courseconfirm/reservationconfirmed.gif) no-repeat; }
.cancel{ border:none;cursor:pointer; margin-top: 20px;background: transparent; text-decoration:underline; color:#cc6600; font-size:16px;}
.evaluate{ background:url(images/courseconfirm/blogevaluate.gif) no-repeat;}
.courseintro{  float: left;width: 60%;margin-top:10px;line-height:18px;text-align: left;}
...
.info_class h3{float:left;margin-left: 280px;color:#cc6600;}
.info_class .htitle{
   color:#000;
}
.info_class table {
...
#reservationbtn{ background: url(images/courseconfirm/coursereservation.gif) no-repeat; width:136px; height:40px; border:0; cursor:pointer;}
.tipsinfo h6{width:100%;}
.tipsinfo{ width:730px; float:left; font-size:16px; color:#333; margin:10px 0px 5px 15px;}
.highlight{ background:url(images/courseconfirm/highlight.gif) left bottom no-repeat; height:35px; padding-left:39px; line-height:35px;}
.succeed{ line-height:35px; color:#cc6600; text-align:center; background:#FFE7CE;}
.succeed img{ vertical-align:middle; display:inline-block;}
#courseinfoshow{  width:770px; margin-left:15px; padding:4px 0 10px 0;}
.topbtn {width:740px; float:left; text-align:center; margin-top:5px;}
.p1{ float:left; font-size:16px; display:block; width:600px; line-height:25px; margin:5px 0px 5px 45px;}
#panelinfo{ width:730px; float:left; margin:10px 0px 5px 15px; font-size:16px;display: none;}
#redirectto{ text-align:center; width:740px; margin:10px 0px 5px 15px; font-size:14px;}
</style>
    ...
    <div id="courseconfirm">
        <div class="blogframe">
            ...
            <div class="tipsinfo">
                ...
                <h6><s:property value="%{getText('label.clourse.selected')}"/>：<s:property value="course.name"/> </br><b style="color:red;" id="msg"><s:property value="purchaseMsg"/></b></h6>
            </div>
            <div id="courseinfoshow">
                ...
                <div class="courseinfo">
                    ...
                    <p><s:property value="%{getText('lable.course.date')}"/>：<b><s:date name="course.startDate" format="%{getText('dateformat')}"/></b>
                    </p>
                </div>
                <div class="courseinfo">
                    <p><s:property value="%{getText('label.course.fee')}"/>：<b><s:if
                        ...
                        <s:if test="course.consumptionType"><s:property
                                ...
                                value="%{getText('label.course.zhiquan')}"/></s:else></s:if><s:else><s:property
                            value="course.charges"/><s:if test="course.consumptionType"><s:property
                            value="%{getText('label.zhibi.true')}"/> </s:if><s:else><s:property
            ...
            <p class="p1"><s:property value="%{getText('label.zhibi.left')}"/>：<b><s:property value="userAccountInfo.zhiBi"/></b></p>
            <p class="p1"><s:property value="%{getText('label.zhiquan.left')}"/>：<b><s:property value="userAccountInfo.zhiQuan"/></b></p>
            ...
            <s:form action="purchaseCourse" method="POST" theme="css_xhtml" id="purchaseCourse_Form">
                ...
                <div class="topbtn">
                    ...
                    <input type="button" class="btn1 cancel" value="<s:property value="%{getText('button.cancel')}" />"/>
                </div>
            </s:form>
            <div id="panelinfo">
                ...
                <p class="p1"><s:property value="%{getText('label.zhibi.true')}"/>:<b id="zhibi"><s:property value="userAccountInfo.zhiBi"/></b></p>
                <p class="p1"><s:property value="%{getText('label.course.zhiquan')}"/>：<b id="zhiquan"><s:property value="userAccountInfo.zhiQuan"/></b></p>
