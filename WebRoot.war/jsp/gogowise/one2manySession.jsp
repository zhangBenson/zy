...
<script type="text/javascript" src="js/virtualVideoRoom.js"></script>
    ...
    <div class="course_msg">
        <div class="course_title">
            <s:a action="voaCourseBlog" target="_blank"><s:param name="course.id" value="courseClass.course.id"/><s:property value="courseClass.course.name"/></s:a>
            ...
            <s:else>
                ...
                <s:else>
                    <s:if test="courseClass.course.teachingNum==1"><s:property value="%{getText('label.tutor.1to1')}"/></s:if>
                    <s:if test="courseClass.course.teachingNum==2"><s:property value="%{getText('label.tutor.1to2')}"/></s:if>
                    <s:if test="courseClass.course.teachingNum==3"><s:property value="%{getText('label.tutor.1to4')}"/></s:if>
                </s:else>
            </s:else>)</span>
        </div>
        <div class="teacher_name">
            <s:property value="%{getText('label.forcast.lecturer')}"/>：
            <s:a action="userBlog" target="_blank"><s:param name="user.id" value="courseClass.course.teacher.id"/><s:property value="courseClass.course.teacher.nickName"/></s:a>
        </div>
        <div class="operate">
            <a id="invite_students" class="submit_btn" href="#invite_student_container" title="<s:property value="%{getText('course.invite.tip.words')}"/>"><s:property value="%{getText('course.invite.friend')}"/></a>
            <a id="report_class" class="submit_btn" title="您可以点击此处举报该课程">举报课程</a>
        </div>
    </div>

    <div class="flash_wrap">
        <s:if test="courseClass.course.teacher.nickName == #session.nickName">
        <iframe scrolling=no frameborder="0"  src="partyMessage.html" id="messages"  name="messages"  style="position:absolute;z-index:2;background-color:#fff;top:355px;left:630px;width:330px; height:200px;"></iframe>
        </s:if>
        <s:else>
        <iframe scrolling=no frameborder="0"  src="partyMessage.html" id="messages"  name="messages"  style="position:absolute;z-index:2;background-color:#fff;top:355px;left:630px;width:285px; height:200px;"></iframe>
        </s:else>
        <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0" name="Girl" width="1004" height="630" id="Girl" wmode="transparent" class="flash_obj">
          <param name="movie" value="<s:property value="flashPatch"/>" />
          <param name="quality" value="high" />
          <param name="wmode" value="transparent" />
          <param name="allowFullScreen" value="true" />
          <embed src="<s:property value="flashPatch"/>" allowFullScreen="true" width="1004" height="630" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" wmode="transparent" name="Girl" id="GirlEmbed"></embed>
        </object>
    </div>
