...
<s:form validate="true" theme="css_xhtml" method="POST" action="identityConfirm">
  ...
  <div id="regright">
      <div id="regrightCont">
      <p class="inputTips"><s:property value="%{getText('email')}" /><span class="errorinfo"  id="yzemail">*</span>
          <span id="emailNotExist" style="display: none;">
               <img src="images/courseImages/error.gif"/><s:property value="%{getText('identity.not.exist.click')}" /><a href="####" onclick="addRepswAndnickName();" style="color:#ff0000;"><s:property value="%{getText('here')}" /></a><s:property value="%{getText('identity.finish.regist')}" />
          </span>
          <s:textfield name="user.email" id="email" type="password" cssClass="text"/>
    </p>
    <p id="addnext"  class="inputTips"><s:property value="%{getText('password')}" /><span class="errorinfo" id="yzpwd">*<s:property value="identityConfirmMsg"/></span><br />
    ...
    <p class="nextButton"><input type="submit" name="button" id="btn1" onclick="return checkForm();" value="<s:property value="%{getText('button.next')}" />" />  <a href="initRepassword.html"><span id="forgotPwd" style="display:inline;">忘记密码？</span></a>
    </p>
   </div>
  </div>
</div>
