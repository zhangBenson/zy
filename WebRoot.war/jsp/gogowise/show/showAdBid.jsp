...
<div class="course_create_container">
    ...
    <s:form action="saveAdvertise" method="POST" enctype="multipart/form-data">
            ...
            <ul class="content_courseInfo">
                <li>
                    ...
                    <div class="option_content">
                        <s:if test="existAds">
                            <s:select name="advertisement.fromAdvertisement.id" list="ads" listKey="key"
                                      id="advertisement_name" cssClass="long_text_field" listValue="value"/>
                        </s:if>
                        <s:else>
                            <s:textfield cssClass="long_text_field" id="bid_brand" name="advertisement.productName"/>
                        </s:else>
                    </div>
                </li>
                <li>
                    ...
                    <div class="option_content"><s:textfield cssClass="long_text_field" id="bid_link"
                                                             name="advertisement.linkAddress"/>
                    </div>
                </li>
                <li>
                    ...
                    <div class="option_content">
                        <s:textarea cssClass="long_text_area" id="bid_description" name="advertisement.description"/>
                    </div>
                </li>
                <li class="long_select_li">
                    ...
                    <div class="option_content"><s:textfield cssClass="short_text_field" id="bid_price"
                                                             name="advertisement.bidPrice"/></div>
                </li>
                <li>
                    <div class="option_tittle"><s:property value="%{getText('show.bidding.ad.logo')}"/></div>
                    <div class="bid_adImg_input_msg input_msg"></div>
                    <div class="option_content">
                        <img src="images/defaultImgs/show.jpg" id="bid_adImg_preview"
                             style="width:240px;height:180px;"/><br/>
                        <input type="file" name="upload" id="bid_adImg_upload"/>
                    </div>
                    <s:hidden id="bid_adImg" name="advertisement.adLogoUrl"/>
                </li>
            </ul>
        </div>
        <div class="create_back_forth">
            <s:submit cssClass="submit_btn" onclick="return checkBidInfoForm();"
                      value="%{getText('show.bidding.confirm')}"/>
        </div>
    </s:form>
</div>
<script type="text/javascript">
    ...
    $(document).ready(function () {
        if ($("#advertisement_name").val() != undefined) {
            $.post("findAdvertise.html", {'advertisement.id': $("#advertisement_name").val()}, function (data) {
    ...
    $("#advertisement_name").change(function () {
        $.post("findAdvertise.html", {'advertisement.id': $(this).val()}, function (data) {
            $("#bid_link").attr('value', data.brandLink);
            $("#bid_description").attr('value', data.brandDescription);
            $("#bid_price").attr('value', data.brandPrice);
            $("#bid_adImg_preview").attr("src", data.brandLogoUrl);
            if ($("#advertisement_name").val() == -1) {
                $("<input type='text' class='long_text_field' id='bid_brand' name='advertisement.productName'/>").replaceAll($("#advertisement_name"));
                $("#bid_adImg_preview").attr("src", "images/defaultImgs/show.jpg");
            }
        });
