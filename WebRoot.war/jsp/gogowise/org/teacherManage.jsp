...
<div class="org_authorization_container">
    <div class="auth_upper">
        <h3><s:text name="org.teacher.management"/></h3>
        <br/>

        <p><s:text name="org.teacher.regret"/></p>
    </div>
    <div class="auth_left">
        ...
        <s:form id="form1" method="POST" validate="false" theme="simple" action="saveOrgUser"
            ...
            <div class="auth_left_2">
                <div class="addlist">
                    <div>
                        <span class="_add_btn" id="add_one_btn"><s:text name="interview.add"/></span>&nbsp;<span><s:property value="%{getText('org.teacher.one')}"/></span>
                        <span class="_add_btn" id="add_more_btn"><s:property value="%{getText('interview.add')}"/>&nbsp;</span>
                        <input type="text" id="more_user_input"
                               style="width:50px;height:26px;margin-left:5px;"/>&nbsp;<s:property value="%{getText('org.teacher.unit')}"/>&nbsp;
...
<script type="text/javascript">
    ...
    function deleteUser(obj, id) {
        ...
        var $userTds = $(obj).parents("tr").children("td");
        var userEmail = $userTds.eq(0).text();
        $.post("deleteUser.html", {"user.email": userEmail, "roleType": 5, 'orgUser.id': id}, function (rd) {
            if (rd.result == 200) {
