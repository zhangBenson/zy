...
<div class="container">
    ...
    <ul class="list-inline">
        <li class="searchItemState"><s:property value="%{getText('course.school')}"/>:<a href="orgBlog.html?org.id=<s:property value="course.organization.id"/>" title="<s:property value="course.organization.nickName"/>"><s:property value="course.organization.schoolName"/></a></li>
        <li class="searchItemState"><s:property value="%{getText('courses.info.lecturer')}"/>:<a href="userBlog.html?user.id=<s:property value="course.teacher.id"/>" title="<s:property value="course.teacher.nickName"/>"><s:property value="course.teacher.nickName"/></a></li>
        <li class="searchItemState"><s:property value="%{getText('label.forcast.enrollment')}"/>:<s:property value="course.observationNum"/></li>
        ...
        <s:if test="course.classes != null && course.classes.size > 0">
            ...
            <tbody>
                ...
                <s:elseif test="course.getForcastClasses().size()>0">
                    <s:iterator value="course.classes" begin="0" end="course.classes.size() - course.getForcastClasses().size() - 1">
                        <tr class="fontRedBold">
                            <td><s:property value="name"/></td>
                            <td><s:property value="nickName"/></td>
                            <td><s:date name="date" format="%{getText('global.display.hour.minute')}"/></td>
        ...
        <div class="row">
            <s:form action="purchaseCourse" method="POST" theme="css_xhtml" id="purchaseCourse_Form">
                ...
                <div class="col-sm-6">
                    ...
                    <input type="button" class="btn btn-danger btn-lg btn-block" value="<s:property value="%{getText('button.cancel')}" />"/>
                </div>
            </s:form>
        </div>
